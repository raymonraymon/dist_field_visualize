...
void draw_rotated_box2d(const Vec2f& centre, float width, float height, float angle) {
   
   glBegin(GL_POLYGON);
   glVertex2fv((centre + rotate(Vec2f(width/2,height/2), angle)).v);
   glVertex2fv((centre + rotate(Vec2f(-width/2,height/2), angle)).v);
   glVertex2fv((centre + rotate(Vec2f(-width/2,-height/2), angle)).v);
   glVertex2fv((centre + rotate(Vec2f(width/2,-height/2), angle)).v);
   glEnd();
...
void draw_tri2d(const Vec2f& a, const Vec2f& b, const Vec2f& c) {
   glBegin(GL_TRIANGLES);
   glVertex2fv(a.v);
   glVertex2fv(b.v);
   glVertex2fv(c.v);
